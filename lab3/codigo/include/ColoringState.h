...
class ColoringState {
    ...
    void assignColor(int vertex, int color, int targetColors);
    bool isValidAssignment(int vertex, int color) const;
    std::vector<int> getAvailableColors(int vertex, int targetColors) const;
    ...
    const AdjList &getUncoloredVertices() const { return uncoloredVertices; }
};
